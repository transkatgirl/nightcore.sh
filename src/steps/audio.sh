...
loudnorm=`ffmpeg -loglevel info -i "$tmpdir/stage1.wav" -af "loudnorm=print_format=summary:tp=-1:i=-14:lra=20" -f null - 2>&1`
...
ffmpeg -i "$tmpdir/stage1.wav" \
	-af "loudnorm=linear=true:tp=-1:i=-14:lra=20:measured_i=$loudnorm_i:measured_lra=$loudnorm_lra:measured_tp=$loudnorm_tp:measured_thresh=$loudnorm_thresh:offset=$loudnorm_offset, \
