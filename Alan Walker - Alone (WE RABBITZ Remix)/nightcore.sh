...
function process_image {
	...
	if [ -f "$audio_title_short" ]; then
		...
		if [ -f "$info_text" ]; then
			ttext="$ttext,drawtext=box=1:boxcolor=black:boxborderw=25:fontcolor=white:font=sans-serif:fontsize=35:text='$(cat $info_text)':x=1205-text_w:y=645-text_h:alpha=$thumbnail_overlay_alpha"
		fi
		ffmpeg $ffloglevelstr -i $image_stage3 -vf "$ttext" $image_stage4
	else
		if [ -f "$info_text" ]; then
			ttext="drawtext=box=1:boxcolor=black:boxborderw=25:fontcolor=white:font=sans-serif:fontsize=35:text='$(cat $info_text)':x=1205-text_w:y=645-text_h:alpha=$thumbnail_overlay_alpha"
			ffmpeg $ffloglevelstr -i $image_stage3 -vf "$ttext" $image_stage4
		else
